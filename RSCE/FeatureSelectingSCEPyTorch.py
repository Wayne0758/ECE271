...
class FeatureSelectingSCE(nn.Module):
    ...
    def postTrain(self, dataLoader, optimizationFunc, learningRate, m, batchSize, numEpochs, verbose):
        ...
        if self.weightCutoff != None:
            ...
            while True:
                for i, (trInput, trOutput) in enumerate(dataLoader):
                    ...
                    if self.l1Penalty != 0 or self.l2Penalty != 0:
                        l1RegLoss, l2RegLoss = torch.tensor([0.0], requires_grad=True).to(self.device), torch.tensor(
                            [0.0], requires_grad=True).to(self.device)
                ...
                if torch.sum(torch.abs(self.hidden[0].weight.data)).item() <= self.weightCutoff:
                    ...
                    if verbose and ((epoch) % 100) == 0:
                        print('Epoch {}, Loss: {:.6f}, L1 Norm of SPL: {:.6f}'.format(epoch, self.epochError[-1],
                                                                                      torch.sum(torch.abs(self.hidden[
                                                                                                              0].weight.data)).item()))
            ...
            for epoch in range(numEpochs):
                ...
                self.epochError.append(np.mean(error))
                if verbose and ((epoch + 1) % (numEpochs * 0.1)) == 0:
                    print('Epoch [{}/{}], Loss: {:.6f}, L1 Norm of SPL: {:.6f}'.format(epoch + 1, numEpochs,
                                                                                       self.epochError[-1], torch.sum(
                            torch.abs(self.hidden[0].weight.data)).item()))
    ...
    def fit(self, trData, trLabels, valData=[], valLabels=[], preTraining=True, optimizationFunc='Adam',
            learningRate=0.001, m=0,
            miniBatchSize=100, numEpochsPreTrn=25, numEpochsPostTrn=100, standardizeFlag=True, cudaDeviceId=0,
            verbose=True):
    ...
    def predict(self, x):
        if len(self.trMu) != 0 and len(
                self.trSd) != 0:  # standarization has been applied on training data so apply on test data
