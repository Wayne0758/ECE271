	...
	def Train(self,dataLoader,optimizationFunc,learningRate,m,batchSize,numEpochs,verbose):
		...
		for epoch in range(numEpochs):
			...
			for d in range(len(self.hLayer)):
				...
				preW.append(self.hidden[1].weight)
				preB.append(self.hidden[1].bias)

			#now set requires_grad =True for all the layers
			self.hidden[1].weight.requires_grad=True			
			self.hidden[1].bias.requires_grad=True
			self.preTrW.append(deepcopy(self.hidden[1].weight.data))
			self.preTrB.append(deepcopy(self.hidden[1].bias.data))
